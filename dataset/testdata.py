...
class GSODataset(Dataset):
    ...
    def __init__(self, opt):
        self.opt = opt
        self.all_items = []
        self.input_path = self.opt.val_data
        self.input_view = self.opt.view
        load_path = os.path.join(self.input_path, "val.json")
        if os.path.exists(load_path):
            ...
            json_file_path = os.path.join(self.input_path, "val.json")
    ...
    def __getitem__(self, idx):
        ...
        c2ws = []
        w2cs = []
        transformsfile = "dataset/transforms_test.json"
        uid = uid.replace("\\","/")
        name = uid.split("/")[-2]+"_"+uid.split("/")[-1]
        with open(transformsfile) as json_file:
            ...
            for idx, frame in enumerate(frames):
                image_path = frame["file_path"]+"/gt.png"
...
class CustomDataset(Dataset):
    ...
    def __init__(self, opt):
        self.opt = opt
        self.all_items = []
        self.input_path = self.opt.test_data
        self.input_view = self.opt.view
        load_path = os.path.join(self.input_path, "test.json")
        if os.path.exists(load_path):
            with open(load_path, 'r') as f:
                self.all_items = json.load(f)
            print(f"Load json file at {load_path}")
            ...
            for idx in os.listdir(self.input_path):
                ...
                for name in os.listdir(idx_path):
                    ...
                    for k_id in os.listdir(obj_path):
                        if os.path.isdir(os.path.join(obj_path, k_id)):
                            sub_idx.append(os.path.join(obj_path, k_id))      
            json_file_path = os.path.join(self.input_path, "test.json")
            with open(json_file_path, 'w') as json_file:
                json.dump(sub_idx, json_file)
            print(f"Generated json file at {json_file_path}")
            with open(load_path, 'r') as f:
    ...
    def __getitem__(self, idx):
        ...
        with open(transformsfile) as json_file:
            ...
            for idx, frame in enumerate(frames):
                ...
                image_path = os.path.join(uid,image_path)
                if idx == 0:
                    input_img,input_normal = self.img_read(image_path,is_known=True)
                else:
                    input_img,input_normal = self.img_read(image_path,is_known=False)
                image_height = input_img.shape[1]
                image_width = input_img.shape[2]
                input_images.append(input_img)
                input_normals.append(input_normal)
                # NeRF 'transform_matrix' is a camera-to-world transform
                c2w = np.array(frame["transform_matrix"],dtype='float32')
